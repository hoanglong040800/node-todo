...
import { buildResponse, empty, validateUUID } from 'utils'

export default async function getTaskDetailController(
	req: Request,
	res: Response,
) {
	try {
		if (!validateUUID(req.params.taskId)) {
			return buildResponse(res, 404, null, 'taskId is not valid')
		}
		...
		const tasks = await db
			...
			.join(
				...
				TABLE.TASKS.FIELDS.USER_ID,
			)
			.where(
				TABLE.USERS.FIELDS.USER_ID,
				'=',
				'24d0c684-30e1-40b0-9b1b-1d1609cc05d5',
			)
